...
import { areAllSelectableDevicesSelected, normalizeSelectedDeviceIds } from "@/lib/deviceSelection";
...
export function DeviceMultiSelect({
  ...
  useEffect(() => {
    ...
    const normalized = normalizeSelectedDeviceIds(Array.from(selected), availableIds);
    const nextSelectAll = areAllSelectableDevicesSelected(normalized, availableIds);
    const selectionChanged =
      normalized.length !== selected.size || normalized.some((deviceId) => !selected.has(deviceId));
    ...
    if (selectAll !== nextSelectAll) {
      setSelectAll(nextSelectAll);
    }
  }, [devices, onChange, selectAll, selected]);
  ...
  const handleDeviceToggle = (deviceId: string, checked: boolean) => {
    ...
    setSelectAll(areAllSelectableDevicesSelected(nextSelected, devices.map((device) => device.id)));
