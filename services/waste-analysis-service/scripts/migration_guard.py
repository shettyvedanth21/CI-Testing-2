"""Advisory-lock guarded Alembic migration runner for waste-analysis-service."""
...
def _ensure_alembic_version_width(conn: pymysql.connections.Connection) -> None:
    with conn.cursor() as cur:
        for table_name in ALEMBIC_VERSION_TABLES:
            cur.execute(
                """
            ...
            if not row:
                print(
                    f"[migration-guard] creating {table_name}.version_num as VARCHAR({ALEMBIC_VERSION_NUM_MIN_LENGTH})",
                    file=sys.stderr,
                )
                cur.execute(
                    f"""
                    CREATE TABLE IF NOT EXISTS `{table_name}` (
                        version_num VARCHAR({ALEMBIC_VERSION_NUM_MIN_LENGTH}) NOT NULL,
            ...
            print(
                f"[migration-guard] widening {table_name}.version_num to VARCHAR({ALEMBIC_VERSION_NUM_MIN_LENGTH})",
                file=sys.stderr,
            )
            cur.execute(
                f"ALTER TABLE `{table_name}` MODIFY version_num VARCHAR({ALEMBIC_VERSION_NUM_MIN_LENGTH}) NOT NULL"
            )


def _build_alembic_database_url() -> str:
    cfg = _parse_database_settings()
    user = quote(str(cfg["user"]), safe="")
    password = quote(str(cfg["password"]), safe="")
    database = quote(str(cfg["database"]), safe="")
    return (
...
def _run_alembic_upgrade() -> int:
    env = os.environ.copy()
    env["DATABASE_URL"] = _build_alembic_database_url()
    try:
        proc = subprocess.run(
