...
ALEMBIC_VERSION_NUM_FLOOR = 255
ALEMBIC_REVISION_PATTERN = re.compile(r'^\s*revision\s*=\s*["\']([^"\']+)["\']', re.MULTILINE)
...
def _required_version_num_length() -> int:
    ...
    for path in ALEMBIC_VERSIONS_DIR.glob("*.py"):
        match = ALEMBIC_REVISION_PATTERN.search(path.read_text(encoding="utf-8"))
        if match:
            longest_revision = max(longest_revision, len(match.group(1)))
    return max(ALEMBIC_VERSION_NUM_FLOOR, longest_revision)


def _ensure_alembic_version_table(conn: pymysql.connections.Connection) -> None:
    ...
    with conn.cursor() as cur:
        ...
        if not table_exists:
            print(
                f"[migration-guard] creating {ALEMBIC_VERSION_TABLE} with version_num VARCHAR({required_length})",
                file=sys.stderr,
            )
            cur.execute(
                f"""
                CREATE TABLE `{ALEMBIC_VERSION_TABLE}` (
                    version_num VARCHAR({required_length}) NOT NULL,
        ...
        print(
            f"[migration-guard] widening {ALEMBIC_VERSION_TABLE}.version_num to VARCHAR({required_length})",
            file=sys.stderr,
        )
        cur.execute(
            f"ALTER TABLE `{ALEMBIC_VERSION_TABLE}` MODIFY version_num VARCHAR({required_length}) NOT NULL"
        )
