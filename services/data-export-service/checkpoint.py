...
class CheckpointRepository:
    ...
    async def device_belongs_to_tenant(self, device_id: str, tenant_id: str) -> bool:
        """Return True only when the device is owned by the tenant scope."""
        query = """
            SELECT 1
            FROM devices
            WHERE device_id = %s
        ...
        async with self._pool.acquire() as conn:
            async with conn.cursor() as cursor:
                await cursor.execute(query, (device_id, tenant_id))
                row = await cursor.fetchone()
                return row is not None

    async def list_tenant_device_ids(
        self,
        tenant_id: str,
        candidate_device_ids: list[str],
    ) -> list[str]:
        """Filter configured export devices down to the authenticated tenant."""
        if not candidate_device_ids:
            return []

        placeholders = ", ".join(["%s"] * len(candidate_device_ids))
        query = f"""
            SELECT device_id
            FROM devices
            WHERE tenant_id = %s
        ...
        async with self._pool.acquire() as conn:
            async with conn.cursor() as cursor:
                await cursor.execute(query, (tenant_id, *candidate_device_ids))
    ...
    async def list_checkpoints_for_retention(
        self,
        *,
        ...
        async with self._pool.acquire() as conn:
            async with conn.cursor(aiomysql.DictCursor) as cursor:
                await cursor.execute(query, (updated_before.replace(tzinfo=None), max(1, int(limit))))
                return list(await cursor.fetchall())

    async def is_latest_reference_for_key(self, *, checkpoint_id: int, s3_key: str | None) -> bool:
        if not s3_key:
            return False
        query = f"""
            SELECT id
        ...
        async with self._pool.acquire() as conn:
            async with conn.cursor(aiomysql.DictCursor) as cursor:
                await cursor.execute(query, (s3_key,))
                row = await cursor.fetchone()
                return bool(row and int(row["id"]) == int(checkpoint_id))

    async def delete_checkpoints_by_ids(self, checkpoint_ids: list[int]) -> int:
        if not checkpoint_ids:
            return 0
        placeholders = ", ".join(["%s"] * len(checkpoint_ids))
        query = f"DELETE FROM {self._table_name} WHERE id IN ({placeholders})"
        if not self._pool:
            raise RuntimeError("Checkpoint repository not initialized")
        async with self._pool.acquire() as conn:
            async with conn.cursor() as cursor:
                deleted = await cursor.execute(query, tuple(int(value) for value in checkpoint_ids))
