...
class S3Writer:
    ...
    async def delete_object_if_exists(self, s3_key: str) -> None:
        if not self._session:
            raise RuntimeError("S3Writer is not initialized")
        ...
        async with self._session.client(
            ...
            try:
                await s3_client.delete_object(Bucket=self.settings.s3_bucket, Key=s3_key)
            except ClientError as e:
                error_code = str((e.response or {}).get("Error", {}).get("Code", ""))
                if error_code in {"404", "NoSuchKey", "NotFound"}:
                    return
                logger.error("Failed to delete S3 object", extra={"s3_key": s3_key, "error": str(e)})
                raise
