...
def test_copilot_api_blocks_requests_without_copilot_entitlement():
    response = _client(granted=False).get("/api/v1/copilot/curated-questions")

    assert response.status_code == 403
    assert response.json()["detail"]["code"] == "FEATURE_DISABLED"


def test_copilot_api_allows_requests_with_copilot_entitlement():
    response = _client(granted=True).get("/api/v1/copilot/curated-questions")

    assert response.status_code == 200
    assert response.json()["starter_questions"]
