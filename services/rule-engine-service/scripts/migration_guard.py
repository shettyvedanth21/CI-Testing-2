"""Advisory-lock guarded Alembic migration runner for rule-engine-service."""
...
ALEMBIC_VERSION_TABLE = "alembic_version_rule_engine"
...
def _ensure_alembic_version_width(conn: pymysql.connections.Connection) -> None:
    with conn.cursor() as cur:
        ...
        if not row:
            print(
                f"[migration-guard] creating {ALEMBIC_VERSION_TABLE}.version_num as VARCHAR({ALEMBIC_VERSION_NUM_MIN_LENGTH})",
                file=sys.stderr,
            )
            cur.execute(
                f"""
