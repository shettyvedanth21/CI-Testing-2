...
TELEMETRY_API_SCHEMA_VERSION = "v1"
TELEMETRY_API_ENRICHMENT_STATUS = EnrichmentStatus.PENDING
TELEMETRY_API_EXCLUDED_EXTRA_FIELDS = {
    "tenant_id",
    "table",
    "result",
    "_start",
...
class TelemetryHistoryQueryError(Exception):
    """Explicit failure while reading historical telemetry from Influx."""
...
class TelemetryHistoryTimeoutError(TelemetryHistoryQueryError):
    """Influx history query timed out."""

    def __init__(self, message: str = "Telemetry history is temporarily unavailable.") -> None:
        super().__init__(
...
class TelemetryPayload(BaseModel):
    ...
    tenant_id: Optional[str] = Field(default=None, description="Tenant identifier from MQTT topic")
...
class TelemetryPoint(BaseModel):
    """Single telemetry data point returned by telemetry APIs.

    Long-term contract:
    - Stored Influx data contributes only `timestamp`, `device_id`, and numeric
      telemetry values.
    - `schema_version` and `enrichment_status` remain API compatibility fields
      synthesized by data-service and are not authoritative Influx columns.
    - Non-contract extras (for example `tenant_id`, `table`, debug markers) must
      not leak into public telemetry responses.
    """
    ...
    schema_version: str = Field(
        default=TELEMETRY_API_SCHEMA_VERSION,
        description="API compatibility field synthesized by data-service.",
    )
    enrichment_status: EnrichmentStatus = Field(
        default=TELEMETRY_API_ENRICHMENT_STATUS,
        description="API compatibility field synthesized by data-service.",
    )

    def to_api_dict(self) -> Dict[str, Any]:
        """Serialize only the intentional public telemetry contract."""
        payload: Dict[str, Any] = {
            "timestamp": self.timestamp,
            "device_id": self.device_id,
            "schema_version": self.schema_version or TELEMETRY_API_SCHEMA_VERSION,
            "enrichment_status": self.enrichment_status or TELEMETRY_API_ENRICHMENT_STATUS,
        }

        for key, value in (self.model_extra or {}).items():
            if key in TELEMETRY_API_EXCLUDED_EXTRA_FIELDS:
                continue
