...
class TelemetryValidator:
    ...
    def _validate_numeric_fields(
        ...
        for key, value in payload.items():
            if key in cls.REQUIRED_FIELDS:
                continue
            if key in cls.NON_NUMERIC_FIELDS:
                continue
            if value is None:
                continue
            try:
    ...
    def _validate_timestamp(
        ...
        max_future_skew = max(int(getattr(settings, "telemetry_max_future_skew_seconds", 300) or 0), 0)
        if parsed > datetime.now(timezone.utc) + timedelta(seconds=max_future_skew):
