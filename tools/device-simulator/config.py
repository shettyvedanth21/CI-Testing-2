...
class SimulatorConfig:
    ...
    mqtt_username: str = ""
    mqtt_password: str = ""
    publish_topic_override: Optional[str] = None
    auth_service_url: str = "http://auth-service:8090"
    mqtt_credential_bootstrap_email: str = ""
    mqtt_credential_bootstrap_password: str = ""
    fault_mode: str = "none"
    log_level: str = "INFO"
    metrics: str = ""
    metrics_json: str = ""
    ...
    def __post_init__(self):
        ...
        valid_fault_modes = {"none", "spike", "drop", "overheating", "phase_imbalance", "power_factor_drop", "load_cycle"}
    ...
    def _parse_metrics(self) -> None:
        ...
        if self.metrics:
            default_ranges = {
                ...
                "voltage_l1": [380.0, 440.0],
                "voltage_l2": [380.0, 440.0],
                "voltage_l3": [380.0, 440.0],
                "current_l1": [0.0, 80.0],
                "current_l2": [0.0, 80.0],
                "current_l3": [0.0, 80.0],
            }
    ...
    def topic(self) -> str:
        ...
        if self.publish_topic_override:
            return self.publish_topic_override
        return f"{self.tenant_id}/devices/{self.device_id}/telemetry"

    @property
    def should_bootstrap_mqtt_credential(self) -> bool:
        return bool(
            self.mqtt_credential_bootstrap_email
            and self.mqtt_credential_bootstrap_password
            and not self.mqtt_username
            and not self.mqtt_password
        )
